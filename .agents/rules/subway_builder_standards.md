...
- **Ruteo Canónico OSRM con `car.lua`:** El enriquecimiento de cohortes (`pops`) debe ejecutarse mediante OSRM (`osrm/osrm-backend:latest`) con perfil estándar de automóvil (`car.lua`) en WSL 2, inyectando `drivingDistance` (metros) y `drivingSeconds` (segundos redondeados). La geometría de ruta (`drivingPath`) es opcional y debe estar desactivada por defecto (`include_driving_path: false`), ya que el motor del juego la ignora en tiempo de ejecución y en metrópolis grandes (> 20k cohortes) las geometrías inflan el archivo a > 500 MB, excediendo el límite rígido de 512 MB de V8 en Electron (`ERR_STRING_TOO_LONG`) y provocando que el juego descarte la demanda por completo (las ciudades oficiales como NYC o Seattle nunca incluyen `drivingPath`). OSRM debe consultarse con `overview=false` cuando `include_driving_path` está apagado para máxima velocidad y bajo consumo de memoria.
